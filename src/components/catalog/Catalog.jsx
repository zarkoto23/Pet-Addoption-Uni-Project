    ...
    >
      ...
      <div className="flex gap-4 overflow-hidden bg-gradient-to-r from-indigo-500/50 via-purple-300/50 to-pink-300/50 p-10 rounded-2xl shadow-2xl">
        {visiblePets.length > 0 ? (
          visiblePets.map((pet) => <CatalogItem key={pet._id} pet={pet} />)
        ) : (
          <NoDataToShowYet />
        )}
      </div>
      ...
      <button
        onClick={handlePrev}
        className="absolute -left-6 top-1/2  bg-white text-indigo font-bold text-xl h-16 w-10 rounded-full shadow-xl flex items-center justify-center hover:scale-110 transition"
      >
        ‹
      </button>
      <button
        onClick={handleNext}
        className="absolute -right-6 top-1/2  bg-white text-indigo font-bold text-xl h-16 w-10 rounded-full shadow-xl flex items-center justify-center hover:scale-110 transition"
      >
        ›
      </button>
    </div>
